...
def query_bm25(query, language = 0):
    if(language == 0):
        top_k_results = rank_bm25_search("./rocksdb/py_rocksdb", query, top_k=1)
    else:
        top_k_results = rank_bm25_search("./rocksdb/cpp_rocksdb", query, top_k=1)
    return top_k_results
